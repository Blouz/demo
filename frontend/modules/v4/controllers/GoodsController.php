...
class GoodsController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        $cate_second_id = RequestHelper::get('cate_second_id', 0, 'intval');
        ...
        if(empty($cate_second_id))
        {
            $map = ['shop_id'=>$this->shop_id, 'cat_id'=>$cat_id, 'status'=>1];
        }
        else
        {
            $map = ['shop_id'=>$this->shop_id, 'cate_second_id'=>$cate_second_id, 'status'=>1];
        }
...
//            exit;
            if (!empty($goods_id)) {
                $models = new Product();
                $goods = $models->getList(['id'=>$goods_id, 'status'=>1], 'id,name,image,bar_code,attr_value,title,description as des');
            }
//            var_dump($goods);
//            exit;
            ...
            foreach ($data_list['list'] as $k => $v) {
                ...
                $goods_arr[$k]['title'] = ArrayHelper::getValue($goods, $v['product_id'].'.title');
                $goods_arr[$k]['des'] = ArrayHelper::getValue($goods, $v['product_id'].'.des');
    ...
    public function actionIndexIos()
    {
        ...
        if (!empty($data_list)) {
            ...
            if (!empty($goods_id)) {
                $models = new Product();
                $goods = $models->getList(['id'=>$goods_id, 'status'=>1], 'id,name,image,bar_code,attr_value,description as des');
            }
            //var_dump($goods);
            $goods = ArrayHelper::index($goods, 'id');
            //var_dump($goods);

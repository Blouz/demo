...
class BaseController extends Controller
{
    ...
    public function init()
    {
        ...
        $url_arr = explode('/', $url);
        $url_arr1 = isset($url_arr[1]) ? $url_arr[1] : 'login';
        $url_arr2 = isset($url_arr[2]) ? $url_arr[2] : '';
        
        $this->saveLog(Common::C('returnLogFile'), var_export($this->params, true));
        if (!empty($this->params['token']) && !empty($this->params['mobile']) && ($url_arr1!='profile' || $url_arr2!='log-out')) {
        ...
        //登录注册及退出登录接口为false
        if (!empty($this->params['mobile']) && $url_arr1 != 'login' && ($url_arr1!='profile' || $url_arr2!='log-out')) {

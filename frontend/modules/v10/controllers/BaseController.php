...
//            $step = User::find()->select(['step'])->where(['mobile'=>$usermobile])->scalar();
//            if ((int)$step != 8) {
//                return $this->returnJsonMsg('6001',[],'没有权限');
//            }
//        }
        $this->mobile = RequestHelper::post('mobile', '', '');
        $this->get_step = RequestHelper::post('get_step', '', '');
        $this->realname = RequestHelper::post('realname', '', '');
        $this->province_id = RequestHelper::post('community_province_id', '', '');
        $this->city_id = RequestHelper::post('community_city_id', '', '');
        $this->area_id = RequestHelper::post('community_area_id', '', '');
        $this->community_id = RequestHelper::post('community_id', '', '');
        $this->community_name = RequestHelper::post('community_name', '', ''); 
        $this->address = RequestHelper::post('address', '', '');
        $this->num = RequestHelper::post('num', '', '');
        $this->fmobile = RequestHelper::post('fmobile', '', '');
        $this->fname = RequestHelper::post('fname', '', '');
        $this->relation = RequestHelper::post('relation', '', '');
    ...
    **/
    public function _addident($value = '', $mobile = '')
    {
        ...
        if(count($level)>0)
        {
            ...
            {
                $gradation = (int)$level[$i]['gradation'];
                if((int)$score>$gradation)
                {
                    continue;
                }
                else
    ...
    public function _authorized($mobile)
    {
        $user_info = User::find()->select(['i500_user.id','i500_user_basic_info.is_pioneer','i500_user_basic_info.id as info_id','i500_user_basic_info.last_community_id'])
                                ->join('inner join','i500_user_basic_info','i500_user.mobile=i500_user_basic_info.mobile')
                                ->where(['i500_user.mobile'=>$mobile])
                                ->asArray()
                                ->one();
       $community_id = $user_info['last_community_id'];
       $pioneer_user = UserBasicInfo::find()->select(['id','mobile','is_pioneer','create_time'])->where(['last_community_id'=>$community_id])->andWhere(['is_pioneer'=>1])->asArray()->one();
       $join_in =  UserBasicInfo::find()->select(['id'])->where(['last_community_id'=>$community_id])->count();
       $create_time = $pioneer_user['create_time'];
       $limit_time = date("Y-m-d H:i:s", strtotime("-21 day"));
       $res = "";
       if($limit_time>=$create_time&&(int)$join_in<10)
       {
//           $user_info = UserBasicInfo::find()->select(['id','last_community_id'])->where(['mobile'=>$mobile])->asArray()->all();
           if(!empty($community_id))
           {            
               $uid = $user_info['info_id'];
               $pioneer = UserBasicInfo::find()->select(['id'])
                                    ...
                                    ->asArray()
                                    ->one();
                $clear = UserBasicInfo::updateAll(['is_pioneer'=>0],['mobile'=>$mobile]);
                if(!empty($pioneer))
                {
                    $info_id = $pioneer['id'];
                    $res = UserBasicInfo::updateAll(['is_pioneer'=>1],['id'=>$info_id]);
                }

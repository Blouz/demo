...
class FriendController extends BaseController
{
    ...
    */
    public function actionFriendList(){
        ...
        $userlist = UserBasicInfo::find()->select([UserFriends::tableName().'.id',UserFriends::tableName().'.status',UserFriends::tableName().'.uid','nickname','avatar','sex','personal_sign','age','message'])
    ...
    */
    ...
    //设置创建者
    private function _set_is_pioneer($mobile) {
        $community_id = UserBasicInfo::find()->select(['last_community_id'])->where(['mobile' => $mobile])->asArray()->scalar();
        if (empty($community_id)) {
            return false;
        }
        $step = User::find()->select(['step'])->where(['mobile'=> $mobile])->asArray()->scalar();
        if (empty($step) || $step<8) {
            return false;
        }

...
{	
    ...
    public function actionChecked()
    {
        ...
        $recieve_time = date("Y-m-d H:i:s",strtotime($current_time ."-7 day"));//7天后到期自动修改收货状态
        $res = UserOrder::updateAll(array('status'=>'2'),'operation_time<:ct AND pay_status=:ps AND status=:st',array(':ct'=>$recieve_time,':ps'=>1,'st'=>5));

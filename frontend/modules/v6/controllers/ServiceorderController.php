...
//        $service_where['servicer_info_status'] = '1';
        ...
        $service_fields = 'uid,category_id,son_category_id,mobile,image,title,price,unit,service_way,description';
...
//        }
		...
		$data['order_sn']=$this->randsn();///生成随机24位订单号
...
//        }
		...
		if($data['order_sn']!==NULL)//
		{
		    ...
		    $servicemob=$service_info['mobile'];
			$sn=$data['order_sn'];
			$pricetot=$data['service_info_price'];
			$remark=$data['service_info_description'];
			...
			$inf['id']=$data['service_id'];
			$inf['mobile']=$servicemob;
			$inf['category_id']=$service_info['category_id'];
			$inf['son_category_id']=$service_info['son_category_id'];
			$inf['content']=$remark;
			$inf['price']=$pricetot;
			$inf['unit']=$data['service_info_unit'];
			$inf['image']=$data['service_info_image'];
			$inf['community_city_id']=$data['community_city_id'];
			$inf['community_id']=$data['community_id'];
			$infomationdata[]=$inf;
			$orderif=json_encode($infomationdata);
			
			$connection->createCommand()->insert('i500_user_order', ['mobile' => $mob,'shop_mobile' =>$servicemob,'order_sn'=>$sn,'total'=>$pricetot,'remark'=>$remark,'create_time'=>$createtime,'order_type'=>1,'community_city_id'=>$comm_city,'community_id'=>$comm_id,'source_type'=>$sourtype,'status'=>1,'order_info'=>$orderif])->execute();
		}

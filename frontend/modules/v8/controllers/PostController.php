...
class PostController extends BaseController
{
    ...
    {   
        ...
        $fields = ['id','mobile','forum_id','content','views','community_id','create_time','thumbs','share_num'];
        $wheres = ['community_id'=>$community_id,'status'=>2,'is_deleted'=>2];
        $forum_id = RequestHelper::post('forum_id', 0, 'intval');
        if (!empty($forum_id)) {
            $wheres["forum_id"] = $forum_id;
        }

        $query = Post::find()->select($fields)->where($wheres)->orWhere(['is_public'=>1])->with(['user'=>function($query) {
        ...
        if (!empty($data['item'])) {
            foreach ($data['item'] as $k => $v) {
                if ($v['forum_id'] != 0) {
                    $category = ServiceCategory::find()->select(['id','name', 'image'])->where(['id'=>$v['forum_id']])->one();
                    $img_paths = \Yii::$app->params['imgUrl'];
                    $data['item'][$k]['image'] = $img_paths.$category['image'];
                    $post['read_volume'] = 0;
                    $post = Post::find()->select(['SUM(views) as read_volume'])->where(['forum_id'=>$v['id']])->asArray()->all();
                    if(empty($post['0']['read_volume'])) {
                        $post['0']['read_volume'] = '0';
                    }
                    $data['item'][$k]['read_volume'] = $post['0']['read_volume'];
                    $count = Post::find()->select(['id'])->where(['forum_id'=>$v['id']])->count();
                    $data['item'][$k]['count'] = $count;
...
//        }
//      
        $this->returnJsonMsg('200',$hotlist, Common::C('code','200','data','[]'));
    }

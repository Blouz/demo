...
class LoginController extends BaseController
{
    ...
    public function actionIndex()
    {
        ...
        if ($type == '1') {
            ...
            $user_fields = 'id,mobile,password,salt,login_count,status,is_deleted';
            $user_info = $user_model->getInfo($user_where, true, $user_fields);
            if (!empty($user_info) && $user_info['is_deleted'] == 2) {
                ...
                //读取爱样品网用户
                $yp_user_model = new YpUser();
                $yp_user_fields = 'id,mobile,password,status';
                $user_info = $yp_user_model->getInfo(['mobile'=>$mobile], true, $yp_user_fields);
                //var_dump($user_info);
                if (empty($user_info)) {
            ...
            $user_info = $user_model->getInfo($user_cond, true, 'id,login_count,salt,status,is_deleted');
...
//        $user_model->updateInfo($user_update_data, $user_update_where);
...
//        $login_log_model->insertInfo($login_log_data);
     ...
     * @return array
     */
    public function actionCheckUserRegister()
    {
        ...
        $where['is_deleted'] = '2';
        $user_model = new User();
        $user_info = $user_model->getInfo($where, true, 'id,status');
        if (empty($user_info)) {
            $this->returnJsonMsg('200', [], Common::C('code', '200'));
        }
        if ($user_info['status'] == '1') {
        ...
        $this->returnJsonMsg('651', [], Common::C('code', '651'));
    }
    ...
    private function _loginPageSendCode($mobile = '')
    {
        ...
        $this->returnJsonMsg('200', ['first_login'=>$first_login], '发送成功');
    }
    ...
    private function _findPwdSendCode($mobile = '')
    {
        ...
        $this->returnJsonMsg('200', ['first_login'=>'2'], '发送成功!');
    }

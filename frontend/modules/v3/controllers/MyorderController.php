...
class MyorderController extends BaseController
{
    ...
    public function actionDetails()
    {
        ...
        $order_fields = '*';
        //$order_fields = ['status','order_sn','address','freight','total'=>'total','shop_id','pay_status','create_time','consignee','consignee_mobile','pay_method_name'=>'pay_method','remark'];

        $info = $order_model->getInfo($order_where, true, $order_fields);
        if (!empty($info)) {
            $info['pay_method_name'] = ArrayHelper::getValue(self::$pay_method, $info['pay_method'], '');
            //$info['pay_method_name']        = $info['pay_method_name'];

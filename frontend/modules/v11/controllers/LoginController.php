...
class LoginController extends BaseController
{
    public function actionIndex()
    {
        ...
        $lng             = RequestHelper::post('lng', '0.00000', '');
        $lat             = RequestHelper::post('lng', '0.00000', '');
        $login_source    = RequestHelper::post('login_source', '1', '');
        $version_number  = RequestHelper::post('version_number', '', '');
        $user_model = new User();
        ...
        if ($type == '1') {
            ...
            if (!empty($user_info) && $user_info['status'] == 2) {
                ...
                if ($password_input != $user_info['password']) {
                    $this->returnJsonMsg('607', [], '账号或密码错误');
                }
            } else {
                //读取爱样品网用户
        ...
        $login_log_data['source'] = $login_source;
        $login_log_data['lng']  = $lng;
        $login_log_data['lat']  = $lat;
        $login_log_model->insertInfo($login_log_data);
        //将版本号写入用户信息表中
        $user_basic_data['version_number'] = $version_number;
        $user_info_model->updateInfo($user_basic_data, ['mobile'=>$mobile]);
...
//        $field['user_id'] = (new Query())->select('id')->from("i500_user")->where("mobile=i500_user_basic_info.mobile");
//        $field['lat'] = (new Query())->select('lat')->from($name.".community_beijing")->where("id=i500_user_basic_info.last_community_id");
        ...
        $community_name = Community::find()->select(['id','name','lng','lat','address'])->where(['id'=>$info['last_community_id']])->asArray()->one();
        ...
        //查询业主群ID
        $group_id = Group::find()->select(['group_id'])->where(['community_id'=>$info['last_community_id'], 'is_deleted'=>2, 'owner_group'=>1])->scalar();

        //家庭组的fmobile fname
        $user_friends = UserFriends::find()->select(['fid'])->where(['uid'=>$mobile,'relation_status'=>2])->asArray()->one();
        $fmobile = empty($user_friends['fid'])?'':$user_friends['fid'];
        $fname = UserBasicInfo::find()->select(['nickname'])->where(['mobile'=>$fmobile])->asArray()->one();
        ...
        $rs_u_info['community_address'] = ArrayHelper::getValue($community_name, 'address', '');
        ...
        $rs_u_info['group_id'] = empty($group_id) ? '' : $group_id;
//        $rs_u_info['user_id'] = ArrayHelper::getValue($community, 'user_id', '');
        $rs_u_info['is_open'] = $is_open;
        $rs_u_info['fmobile'] = $fmobile;
        $rs_u_info['fname'] = empty($fname['nickname'])?'':$fname['nickname'];
        ...
        if(!empty($rest['step']) && $rest['step'] == 8) {
            ...
            if (empty($group)) {
                ...
                if (!empty($model) && $model['ActionStatus'] == 'OK') {
                    ...
                    $group_model->community_id = $info['last_community_id'];
                    $group_model->name = $data_name;
                    $group_model->group_id = $model['GroupId'];
                    $group_model->image = Common::C('defaultGroup');
                    $group_model->desc = '';
                    $group_model->is_deleted = 2;
                    $group_model->owner_group = 1;
                    $group_model->create_time = date('Y-m-d H:i:s');
                    $group_model->source = 1;
                    $res = $group_model->save(false);

                    $user_mobile_data = [];
                    if ($res) {
                        $user_mobile_data[] = $user_id;
                        $join_in = TxyunHelper::Join_group($model['GroupId'], $user_mobile_data);

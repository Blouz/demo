...
class BankHelper
{
     ...
     * @return array
     */
    public static function getBankInfoOld($bankcardnum) {
    	$bankList = self::$bankList;
	 	$card_8 = substr($bankcardnum, 0, 8);
	    if (isset($bankList[$card_8])) {
	        return explode('-', $bankList[$card_8]);
	        
	    }
	    $card_6 = substr($bankcardnum, 0, 6);
	    if (isset($bankList[$card_6])) {
	        return explode('-', $bankList[$card_6]);
	    }
	    $card_5 = substr($bankcardnum, 0, 5);
	    if (isset($bankList[$card_5])) {
	        return explode('-', $bankList[$card_5]);
	    }
	    $card_4 = substr($bankcardnum, 0, 4);
	    if (isset($bankList[$card_4])) {
	        return explode('-', $bankList[$card_4]);
	    }
	    return [];
    }
     ...
     * @return array
     */
    public static function getBankInfo($bankcardnum) {
    	$url = "https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?_input_charset=utf-8&cardNo=".$bankcardnum."&cardBinCheck=true";
    	...
    	if(!empty($result->validated)){
    		$bankInfo = array(
                ...
                'bankColor'    => isset(self::$bankColor[$result->bank]) ? self::$bankColor[$result->bank] : 4,   // 银行颜色
            );
    	}
	    return $bankInfo;
    }
